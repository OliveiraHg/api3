...
function getVisitor() {
  const fs = require('fs');
  const ceemde = JSON.parse(fs.readFileSync('./visits.json'));
  var xhr = new XMLHttpRequest();

  xhr.onloadend = function(){
    ceemde[0].visits += 1;
    fs.writeFileSync('./visits.json', JSON.stringify(ceemde));
    document.getElementById("visits").innerHTML = ceemde[0].visits;
  };
}
...
txt += "<p><b>Fornecedor</b>: " + navigator.vendor + "</p>";
...
txt += "<p><b>Agente de usuário</b>: <br>" + navigator.userAgent + "</p>";
...
if (hours >= 17 || hours <= 3) {
  ...
  Sayings = "<span class='text-light'>Bom dia 🌞</span>"
} else {
  Sayings = "<span class='text-light'>Boa tarde 🌞</span>"
}
...
var batteryLevel = document.getElementById("batteryLevel");
	...
	const Image = ["./../image/bryan.png","./../image/bryan.png","./../image/bryan.png"]
